...
<link rel="stylesheet" href="${path}/resources/css/userList-withdraw.css">
</head>
<body>
	<div class="content">
	<div class="title" id= "withdrawTitle" onclick="location.href='/manageUser'">탈퇴회원관리</div>
	<div class="title" id= "blockTitle" onclick="location.href='/blockUser'">정지회원관리</div>
	<hr>
		...
		<table class="userTable">
			...
			<td>${userInfoVO.uiAge}대 이상</td>
			</c:if>
			<c:if test="${userInfoVO.uiAge<50}">
			<td>${userInfoVO.uiAge}대</td>
			</c:if>
			<td>${userInfoVO.uiDel}</td>
			<td>
			<button class="Btn" onclick="location.href='/withdrawCancle/${userInfoVO.uiNum}'">탈퇴취소</button></td>
			</tr>
			</c:forEach>
		</table>
		<br>
